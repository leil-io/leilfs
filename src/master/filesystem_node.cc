...
void FilesystemNodeOperationsBase::getGoalRecursive(const FilesystemOperationContext &fsOpContext,
	...
	if (node->type == FSNodeType::kFile || node->type == FSNodeType::kTrash ||
		...
		if (!GoalId::isValid(node->goal)) {
			...
			// Persist the fix: updateNode() emits on signal backends; on MDS it is a no-op during
			// a read-only GETGOAL walk (no read-write transaction), matching the prior behavior.
			updateNode(fsOpContext, node);
		}
		...
		if (!GoalId::isValid(node->goal)) {
			...
			fsnodes_update_checksum(node);
			// Persist the fix: updateNode() emits on signal backends; on MDS it is a no-op during a
			// read-only GETGOAL walk (no read-write transaction), matching the prior behavior. Still
			// gated on an actual fix (the enclosing invalid-goal check), not every visited node.
			updateNode(fsOpContext, node);
		}
		...
		if ((node->mode & (EATTR_NOOWNER << EATTR_BIT_OFFSET)) == 0 && uid != 0 &&
			...
			if ((smode & SMODE_TMASK) == SMODE_SET && node->goal != goal) {
				if (node->type != FSNodeType::kDirectory) {
					...
					node->goal = goal;
					// Persisted by the tail updateNode(node) below (both backends).
					(*modifiedINodesOut)++;
				}
