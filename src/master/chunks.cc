...
struct ChunkPart {
	...
	bool beingWritten;  /*!< Indicates if the chunk part is being written, i.e locked. */

	ChunkPart() : version(0), type(), csid(0), state(INVALID), beingWritten(false) {}
...
uint8_t chunk_multi_modify(uint64_t currentChunkId, uint32_t *lockid, uint8_t goal,
                           bool usedummylockid, bool quotaExceeded, uint8_t *operation,
                           uint64_t *targetChunkId, uint32_t minServerVersion = 0) {
...
void chunk_write_end_status(Chunk *chunk, ChunkPartType chunkType, uint8_t status,
	...
	if (!anyCopyBeingWritten && chunk->interrupted) {
		if (chunk->isWritable()) {
			...
			safs::log_err("{}: chunk {} is not writable. Lost chunk?", __func__, chunk->chunkid);
		}
	}
