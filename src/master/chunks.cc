...
void chunk_finalize_failed_operation(Chunk *c) {
	...
	// Every part answered and the chunk is idle again, so the backend publishes what survived.
...
size_t chunk_reclaim_unreferenced(size_t budget, const std::function<bool(uint64_t)> &hasRecord) {
	// Resumes where the previous call stopped, inside a bucket too, so a small budget still
	// covers the whole table and one long bucket cannot exceed it.
	static uint32_t bucket = 0;
	static size_t indexInBucket = 0;
	size_t visited = 0;
	for (uint32_t step = 0; step < static_cast<uint32_t>(kChunkHashSize) && visited < budget;
	     ++step) {
		const auto &chunks = gChunksMetadata->chunkhash[bucket];
		while (indexInBucket < chunks.size()) {
			if (visited == budget) { return visited; }
			Chunk *c = chunks[indexInBucket++];
			++visited;
