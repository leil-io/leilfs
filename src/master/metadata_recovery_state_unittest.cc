...
TEST_F(EdgeRecoveryStateTest, RestoresInodeKeyedDetachedPaths) {
	...
	// TrashPathKey contains the expiry timestamp as well as the inode. Non-default values ensure
	// rollback reconstructs and uses the complete key for direct lookup.
	trashNode->ctime = 1234;
	trashNode->trashtime = 5678;
	const TrashPathKey expectedTrashKey(trashNode);
	...
	EXPECT_EQ((*gMetadata->trash.begin()).first.timestamp, expectedTrashKey.timestamp);
