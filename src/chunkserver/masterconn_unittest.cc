...
struct ReconciliationHarness {
	...
	void initialize(bool sendInventory = false) {
		...
		jobs = std::make_shared<MasterJobPool>("reconcile", 1, 10, 1, descriptors);
		state.jobDescriptors[0] = descriptors[0];
		replications = std::make_shared<MasterJobPool>("reconcile-repl", 1, 10, 1, descriptors);
		state.replicationDescriptors[0] = descriptors[0];
