...
TEST(MasterConnectionTests, DestroyedConnectionDisarmsPendingReplies) {
	...
	auto pool = std::make_shared<MasterJobPool>("term-test", 1, 10);
...
TEST(MasterConnectionTests, LockJobsUseOriginatingListener) {
	auto pool = std::make_shared<MasterJobPool>("connection-test", 1, 10);
	ASSERT_GE(pool->allocateListener(1), 0);
...
TEST(MasterConnectionTests, RetiredListenerWaitsForDeferredJob) {
	MasterJobPool pool("connection-test", 1, 10);
	const std::vector<int> descriptors{pool.allocateListener(0), pool.allocateListener(1)};
...
TEST(MasterConnectionTests, ConfiguredDisconnectKeepsLockCompletionAttached) {
	auto jobPool = std::make_shared<MasterJobPool>("configured-lock", 1, 10);
	auto replicationPool = std::make_shared<MasterJobPool>("configured-lock-repl", 1, 10);
...
void checkAdmissionFailure(bool failSecondPool) {
	auto jobPool = std::make_shared<MasterJobPool>("admission", 1, 10);
	auto replicationPool = std::make_shared<MasterJobPool>("admission-repl", 1, 10);
	const std::vector<int> jobDescriptors{jobPool->allocateListener(0)};
	const std::vector<int> replicationDescriptors{replicationPool->allocateListener(0)};
...
TEST(MasterConnectionTests, LegacyCompletionWaitsAfterPollErrorUntilReconnect) {
	...
	ModuleMock master;
	master.init();
	auto jobPool = std::make_shared<MasterJobPool>("legacy-poll", 1, 10);
	auto replicationPool = std::make_shared<MasterJobPool>("legacy-poll-repl", 1, 10);
	const std::vector<int> jobDescriptors{jobPool->allocateListener(0)};
	const std::vector<int> replicationDescriptors{replicationPool->allocateListener(0)};
	MasterConn connection("localhost", "9420", "default", jobPool, replicationPool);
...
TEST(MasterConnectionTests, DiscoveredCompletionDrainsWhileOffline) {
	auto jobPool = std::make_shared<MasterJobPool>("discovered-poll", 1, 10);
	auto replicationPool = std::make_shared<MasterJobPool>("discovered-poll-repl", 1, 10);
	const std::vector<int> jobDescriptors{jobPool->allocateListener(0),
	                                      jobPool->allocateListener(1)};
	const std::vector<int> replicationDescriptors{replicationPool->allocateListener(0),
	                                              replicationPool->allocateListener(1)};
...
TEST(MasterConnectionTests, IdentityCompletionDrainsAfterReconnectResetsInventoryPolicy) {
	auto jobPool = std::make_shared<MasterJobPool>("identity-poll", 1, 10);
	auto replicationPool = std::make_shared<MasterJobPool>("identity-poll-repl", 1, 10);
	const std::vector<int> jobDescriptors{jobPool->allocateListener(0)};
	const std::vector<int> replicationDescriptors{replicationPool->allocateListener(0)};
	MasterConn connection("localhost", "9420", "default", jobPool, replicationPool);
	connection.setMode(ConnectionMode::CONNECTED);
...
TEST(MasterConnectionTests, MovedLockCompletionDrainsWhileConfiguredOffline) {
	auto jobPool = std::make_shared<MasterJobPool>("moved-lock", 1, 10);
	auto replicationPool = std::make_shared<MasterJobPool>("moved-lock-repl", 1, 10);
	const std::vector<int> jobDescriptors{jobPool->allocateListener(0),
	                                      jobPool->allocateListener(1)};
	const std::vector<int> replicationDescriptors{replicationPool->allocateListener(0),
	                                              replicationPool->allocateListener(1)};
	MasterConn configured("localhost", "9420", "default", jobPool, replicationPool);
	configured.setMode(ConnectionMode::CONNECTED);
	configured.setSendInventory(false);
	configured.setMode(ConnectionMode::KILL);
	masterconn_close_connection(*jobPool, *replicationPool, configured, 0);

	// The configured connection is gone when a peer retires with a client still holding its lock.
	const auto chunkType = slice_traits::standard::ChunkPartType();
	constexpr uint64_t chunkId = 123;
	uint32_t completionCount = 0;
	ASSERT_TRUE(jobPool->startChunkLock({}, nullptr, chunkId, chunkType, 1));
	jobPool->detachLockJobs(
	    [&](ChunkWithType, uint32_t) { return [&](uint8_t, void *) { ++completionCount; }; }, 1, 0);
	jobPool->endChunkLock(chunkId, chunkType, SAUNAFS_ERROR_IO);

	MasterConnCompletionPollPositions positions;
	std::vector<pollfd> descriptors;
	masterconn_add_completion_descriptors(configured, jobDescriptors[0], replicationDescriptors[0],
	                                      positions, descriptors);
	configured.providePollDescriptors(descriptors, false);
	ASSERT_GE(positions.job, 0);
	ASSERT_EQ(poll(&descriptors[positions.job], 1, 1000), 1);
	masterconn_serve_connection(*jobPool, *replicationPool, configured, 0, positions, descriptors);
	EXPECT_EQ(completionCount, 1U);
	EXPECT_TRUE(jobPool->isListenerIdle(0));
	EXPECT_TRUE(jobPool->isListenerIdle(1));
}
...
TEST(MasterConnectionTests, ListenerTablePublishesToRunningWorkers) {
	...
	MasterJobPool pool("race-test", 4, 1000);

	std::vector<int> notifiers{pool.allocateListener(0)};
...
TEST(MasterConnectionTests, DisconnectLeavesOtherListenerJobInFlight) {
	...
	auto jobPool = std::make_shared<MasterJobPool>("isolation-test", 2, 10);
	auto replicationPool = std::make_shared<MasterJobPool>("isolation-repl", 1, 10);
	const std::vector<int> jobDescriptors{jobPool->allocateListener(0),
	                                      jobPool->allocateListener(1)};
	ASSERT_GE(replicationPool->allocateListener(1), 0);
	MasterConn survivor("localhost", "9420", "default", jobPool, replicationPool);
...
TEST(MasterConnectionTests, UnsentNegativeReportsSurviveDisconnect) {
	auto jobPool = std::make_shared<MasterJobPool>("report-test", 1, 10);
	auto replicationPool = std::make_shared<MasterJobPool>("report-repl", 1, 10);
	MasterConn connection("localhost", "9420", "default", jobPool, replicationPool);
	connection.setMode(ConnectionMode::CONNECTED);
	connection.setSendInventory(false);
	const auto chunkType = slice_traits::standard::ChunkPartType();
...
TEST(MasterConnectionTests, LateDeleteCompletionAlwaysReportsTheLoss) {
	...
	MasterJobPool jobPool("late-delete", 1, 10);
	MasterJobPool replicationPool("late-delete-repl", 1, 10);
	MasterConn connection("localhost", "9420", "default", nullptr, nullptr);
	connection.setMode(ConnectionMode::CONNECTED);
	connection.setMode(ConnectionMode::KILL);
	masterconn_close_connection(jobPool, replicationPool, connection, 0);
