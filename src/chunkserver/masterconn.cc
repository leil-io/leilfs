...
void masterconn_check_hdd_reports() {
	...
	if (eptr->isRegistered()) {
		...
		if (eptr->sendsInventory()) {
			...
			// so the queue is dropped instead of sent.
			hddDiscardNewChunks();
		}
	}
}
...
int masterconn_init_threads(void) {
	...
	try {
		...
		gConnectionState.jobDescriptors[0] = gJobPool->allocateListener(0);
	...
	try {
		...
		gReplicationJobPool = std::make_shared<MasterJobPool>(
		    "ma_repl", gReplicationNumberOfWorkers, kMaxBackgroundJobsCount);
		gConnectionState.replicationDescriptors[0] = gReplicationJobPool->allocateListener(0);
	} catch (const std::exception &e) {
