...
#define SAU_ACL_POSIX_MODE_ALL (SAU_ACL_POSIX_MODE_READ | SAU_ACL_POSIX_MODE_WRITE \
	| SAU_POSIX_MODE_EXEC)
 ...
 */
typedef int (*sau_lock_register_interrupt_t)(struct sau_lock_interrupt_info *info,
                                             void *priv);
 ...
 *
 *  \warning Creating context with secondary groups involves calling sau_update_groups
 ...
 *  sau_update_groups(ctx, instance), context is bound to instance it was registered with
 ...
 * \param instance instance returned from sau_init
 * \param ctx context to be updated
 * \param gids array of new group ids to be set
 * \param gid_num length of gids array
 * \return 0 on success, -1 if failed, sets last error code (check with sau_last_err())
 */
...
int sau_mknod(sau_t *instance, sau_context_t *ctx, sau_inode_t parent,
              const char *path, mode_t mode, dev_t rdev, struct sau_entry *entry);
 ...
 * \return 0 on success, -1 if failed, sets last error code (check with sau_last_err())
 */
int sau_readdir(sau_t *instance, sau_context_t *ctx, struct sau_fileinfo *fileinfo,
                off_t offset, size_t max_entries, struct sau_direntry *buf,
                size_t *num_entries);

/*! \brief Destroy dir entries placed in an array
 * \param buf buf argument to previous successful call to sau_readdir
 * \param num_entries positive *num_entries value after respective sau_readdir() call
 */
 ...
 * \return true link size on success, -1 if failed, sets last error code (check with sau_last_err())
 */
 ...
 * \param entries out_entries argument to previous call to either sau_readreserved or sau_readtrash
 ...
 * \param ctx context returned from sau_create_context
 * \param inode inode of a file
 * \param goal_name buffer to be filled with goal, must be at least SAUNAFS_MAX_GOAL_NAME long
 * \return 0 on success, -1 if failed, sets last error code (check with sau_last_err())
 */
 ...
 * \return 0 on success, -1 if failed, sets last error code (check with sau_last_err())
 * \post retrieved chunks information should be freed with sau_destroy_chunks_info call
 */
 ...
 * \param buffer buffer used in a successful sau_get_chunks_info call
 */
 ...
 * \return 0 on success, -1 if failed, sets last error code (check with sau_last_err())
 * \post retrieved chunkservers information should be freed with sau_destroy_chunkservers_info call
 */
 ...
 * \param buffer buffer used in a successful sau_get_chunkservers_info call
 */
 ...
 * \return 0 on success, -1 if failed, sets last error code (check with sau_last_err())
 * \post interrupt data registered by handler can be later passed to sau_setlk_interrupt
 *       in order to cancel a lock request
 */
int sau_setlk(sau_t *instance, sau_context_t *ctx, sau_fileinfo_t *fileinfo,
              const sau_lock_info_t *lock, sau_lock_register_interrupt_t handler,
              void *priv);
