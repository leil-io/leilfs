...
inline void serialize(std::vector<uint8_t>& destination,
		uint64_t chunkId, uint32_t chunkVersion, ChunkPartType chunkType,
		const std::vector<ChunkTypeWithAddress>& chain, bool expectWriteFlush) {
	serializePacket(destination, SAU_CLTOCS_WRITE_INIT, kECChunksWithWriteFlush,
			chunkId, chunkVersion, chunkType, chain, expectWriteFlush);
}
...
inline void deserialize(const uint8_t *source, uint32_t sourceSize, uint64_t &chunkId,
                        ...
                        std::vector<ChunkTypeWithAddress> &chain, bool &expectWriteFlush) {
