...
# CS 0 has a disk which will fail during the test when writing any files.
USE_RAMDISK=YES \
	CHUNKSERVERS=4 \
	CHUNKSERVER_EXTRA_CONFIG="HDD_TEST_FREQ = 10000" \
	CHUNKSERVER_0_DISK_0="$RAMDISK_DIR/pwrite_EIO_hdd_0" \
	...
	MASTER_CUSTOM_GOALS="8 ec21: \$ec(2,1)" \
	setup_local_empty_saunafs info

# Test ec goals to check the behavior when using write buffering.
goals=(3 ec21)
...
for goal in "${goals[@]}"; do
	...
	FILE_SIZE=1234 assert_success file-generate "$dir"/small_{1..10}
	FILE_SIZE=1M   assert_success file-generate "$dir"/big_{1..10}

	# Assert that exactly disks marked "pwrite_EIO" are marked as damaged
	sleep 1
	list=$(saunafs_admin_master_no_password list-disks)
	assert_equals 4 "$(wc -l <<< "$list")"
	assert_awk_finds_no '(/EIO/ && $4 != "yes") || (!/EIO/ && $4 != "no")' "$list"

	# Assert that data is replicated to chunkservers 1, 2, 3 and no chunk is stored on cs 0
	for f in "$dir"/*; do
		assert_eventually_prints "" "saunafs fileinfo '$f' | grep ':${info[chunkserver0_port]}'"
		assert_eventually_prints 3 "saunafs fileinfo '$f' | grep copy | wc -l"
		assert_success file-validate "$f"
	done
done
