...
# The shadow must converge from the metadata image saved above. A changelog application failure
# asks the primary for a fresh image, which can let the test resynchronize from already-current
# state and pass without exercising reconstruction of the saved free-inode state.
