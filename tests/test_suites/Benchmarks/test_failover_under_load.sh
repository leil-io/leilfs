...
# stream records: ops, max per-op latency (ms), errors, and per-op timelines
# so the stall around the promotion is attributable.
...
reader_loop() {
	...
	while [[ ! -f "$stop_file" ]]; do
		...
		local t0
		t0=$(date +%s%N)
		dd if="$file" bs=16 count=1 > /dev/null 2>&1 || errors=$((errors + 1))
		local dt=$(( ($(date +%s%N) - t0) / 1000000 ))
		(( dt > worst )) && worst=$dt
		ops=$((ops + 1))
	done
	echo "$ops $worst $errors" > "$out"
}
...
writer_loop() {
	...
	while [[ ! -f "$stop_file" ]]; do
		...
		local t0
		t0=$(date +%s%N)
		if echo "payload $ops" > "$file" 2>/dev/null &&
...
# ---- Report ----
for ((m = 0; m < MOUNT_COUNT; ++m)); do
	read -r ops worst errors < "$metrics_dir/stream_$m"
