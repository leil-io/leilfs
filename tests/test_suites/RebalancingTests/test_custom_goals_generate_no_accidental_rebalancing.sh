...
status() {
	saunafs-admin list-chunkservers localhost "${info[matocl]}"
}
...
CHUNKSERVERS=6 \
	...
	CHUNKSERVER_EXTRA_CONFIG="HDD_LEAVE_SPACE_DEFAULT = 0MiB`
			`|HDD_TEST_FREQ = 10000`
			`|MAGIC_DEBUG_LOG = $TEMP_DIR/log" \
	MASTER_EXTRA_CONFIG="ACCEPTABLE_DIFFERENCE = 0.03`
