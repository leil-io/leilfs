...
#
# Goal of test:
# Verify that fio tool performs correctly random reads on top of the Ganesha client.
#
...
echo ""
echo "Running fio random reads on top of Ganesha Client:"
cd ${TEMP_DIR}/mnt/ganesha

# Run fio random read test on top of Ganesha Client
fio --name=fiotest_random_read --directory=${TEMP_DIR}/mnt/ganesha \
    --size=200M --rw=randread --numjobs=5 --ioengine=libaio --group_reporting \
