...
#
# Goal of test:
# Verify that fio tool performs correctly random writes on top of the Ganesha client.
#
...
echo ""
echo "Running fio random write on top of Ganesha Client:"
cd ${TEMP_DIR}/mnt/ganesha

# Run fio random write test on top of Ganesha Client
fio --name=fiotest_random_write --directory=${TEMP_DIR}/mnt/ganesha \
    --size=200M --rw=randwrite --numjobs=5 --ioengine=libaio --group_reporting \
