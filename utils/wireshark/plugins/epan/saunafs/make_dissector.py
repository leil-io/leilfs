...
class PacketDissectionVariant(object):
    def __init__(self, message):
        ...
        self.elements = []
        self.finished = False
        extra_size = 4 if message[:4] == 'SAU_' else 0
    ...
    def print_method(self, funcname):
        ...
        for (type, name, start, length) in self.elements:
            ...
            if length is None:
                ...
                length = -1
            print('        proto_tree_add_item(tree, hf_saunafs_{}, tvb, {}, {}, {});'.format(
...
dissectinfo = {}
dictionaries = {'type': []}
packet_regexp = re.compile(r'(SAU_)?(AN|CS|CL|MA|ML|TS)TO(AN|CS|CL|MA|ML|TS)_[A-Z0-9_]+$')
...
for line in sys.stdin:
    try:
        ...
        # Default condition for SAU_* packets is version==0
        if command[:4] == 'SAU_' and not condition_added:
...
print('static dissector_handle_t saunafs_handle;')
print('static int proto_saunafs = -1;')
print('static gint ett_saunafs = -1;')
print('static range_t *tcp_ports_saunafs = NULL;')
for field in sorted(field_types):
    print('static int hf_saunafs_{} = -1;'.format(field))
print()
...
for message in sorted(dissectinfo):
    ...
    if len(dissectinfo[message]) == 0:
        ...
        parsed_part_len = 12 if message[:4] == 'SAU_' else 8
        print('    col_append_str(pinfo->cinfo, COL_INFO, " (info not available)");')
        print('    if (length > {}) {{'.format(parsed_part_len - 8))
        print('        proto_tree_add_item(tree, hf_saunafs_data, tvb, {}, -1, ENC_NA);'.format(parsed_part_len))
        print('    }')
    else:
...
static int dissect_saunafs_message(tvbuff_t *tvb, packet_info *pinfo, proto_tree *tree, void *data _U_) {
    ...
    col_set_str(pinfo->cinfo, COL_PROTOCOL, "SaunaFS");
    col_clear(pinfo->cinfo, COL_INFO);
    col_add_fstr(pinfo->cinfo, COL_INFO, "%-30s",
                 val_to_str(type, dictionary_type, "UNKNOWN(0x%02x)"));

    if (tree) {
        ti = proto_tree_add_item(tree, proto_saunafs, tvb, 0, -1, ENC_NA);
        proto_item_append_text(ti, ", %s",
                val_to_str(type, dictionary_type, "UNKNOWN(0x%02x)"));

        saunafs_tree = proto_item_add_subtree(ti, ett_saunafs);
        proto_tree_add_item(saunafs_tree, hf_saunafs_type, tvb, 0, 4, ENC_BIG_ENDIAN);
        proto_tree_add_item(saunafs_tree, hf_saunafs_length, tvb, 4, 4, ENC_BIG_ENDIAN);
        if (type >= 1000 && type <= 2000) {
            proto_tree_add_item(saunafs_tree, hf_saunafs_version, tvb, 8, 4, ENC_BIG_ENDIAN);
        }
    }
...
static void register_tcp_port(guint32 port, gpointer ptr _U_) {
    if (port != 0) {
        dissector_add_uint("tcp.port", port, saunafs_handle);
    }
}
...
{
    if (port != 0) {
        dissector_delete_uint("tcp.port", port, saunafs_handle);
    }
}

void proto_reg_handoff_saunafs(void) {
    static gboolean saunafs_initialized = FALSE;
    static range_t *tcp_ports_saunafs_copy  = NULL;

    if (!saunafs_initialized) {
        saunafs_handle = create_dissector_handle(dissect_saunafs, proto_saunafs);
        saunafs_initialized = TRUE;
    } else {
        if (tcp_ports_saunafs_copy != NULL) {
            range_foreach(tcp_ports_saunafs_copy, unregister_tcp_port, NULL);
            g_free(tcp_ports_saunafs_copy);
        }
    }
    tcp_ports_saunafs_copy = range_copy(wmem_epan_scope(), tcp_ports_saunafs);
    range_foreach(tcp_ports_saunafs_copy, register_tcp_port, NULL);
}

void proto_register_saunafs(void) {
    module_t *saunafs_module = NULL;

    /* Subtree */
    static gint *ett[] = { &ett_saunafs };
...
for field in sorted(field_types):
    ...
    print('        {{&hf_saunafs_{}, {{'.format(field))
    print('            "{0}", "saunafs.{0}",'.format(field))
    print('            {}, {}, {}, 0, NULL, HFILL}}'.format(typestr, basestr, dictstr))
    print('        },')
print('''};
    proto_saunafs = proto_register_protocol("SaunaFS Protocol", "SaunaFS", "saunafs");
    proto_register_field_array(proto_saunafs, hf, array_length(hf));
    proto_register_subtree_array(ett, array_length(ett));
    range_convert_str(wmem_epan_scope(), &tcp_ports_saunafs, "9419-9422", MAX_TCP_PORT);
    saunafs_module = prefs_register_protocol(proto_saunafs, proto_reg_handoff_saunafs);
    prefs_register_range_preference(saunafs_module, "tcp_ports",
                 "SaunaFS TCP Ports",
                 "The TCP ports for the SaunaFS Protocol",
                 &tcp_ports_saunafs, 65535);
}''')
