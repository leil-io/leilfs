...
leil-goals.cfg - replication goals configuration file

== DESCRIPTION

The file *leil-goals.cfg* contains definitions of the replication goals.
...
- which is the maximal default number of copies. The purpose of leil-goals.cfg is
...
(leil-setgoal(1), leil-getgoal(1)). 'name' can consist of up to 32 alphanumeric
...
leil-chunkserver.cfg(5). 'label' can consist of up to 32 alphanumeric characters:
a-z, A-Z, 0-9, _.
...
Note that changing the definition of a goal in leil-goals.cfg affects all files
