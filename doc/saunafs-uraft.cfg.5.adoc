...
(example: eth0).
Important: If the network interface used for the floating IP becomes unavailable,
the failover mechanism will not restore it. As a result, the floating IP cannot
be recovered, causing SaunaFS services that depend on it to become unavailable.
...
If set to `0` (Default value), the node can both vote in LEADER elections and be
elected as the new leader.
If set to `1`, the node acts as an elector only: it participates in voting for a
new leader but cannot itself be elected leader.
When running in elector-only mode, the node does not requires the metadata service,
which reduces its memory consumption.
