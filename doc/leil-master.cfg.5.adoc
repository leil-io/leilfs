...
protocol (default is 1000, range 1..100000)
...
chunkservers reconnect at once (e.g. right after a failover). 0 means
...
To choose a value, measure the registration rate with the budget unset and
use a fraction of that rate. Avoid setting the value too low, as chunk
registration after a failover could then take minutes instead of seconds.
