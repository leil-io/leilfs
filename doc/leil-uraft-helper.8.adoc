...
If `leil-master` is wedged or slow to connect, *leil-uraft* may time out and log
`(checkNodeStatus): Isalive timeout reported after X ms.`, where `X` is the value
of `URAFT_GETVERSION_TIMEOUT`. This timeout is used for helper commands expected
to be fast, such as `leil-uraft-helper metadata-version` and `leil-uraft-helper isalive`.
