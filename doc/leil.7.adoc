...
leil, lfs - a networking, distributed, highly available file system

== DESCRIPTION

LeilFS is a networking, highly available, distributed file system. It spreads
data over several physical localisations (servers), which are visible to a user
as one resource. For standard file operations LeilFS acts as other Unix-alike
...
locally, not necessarily on LeilFS) and hard links (different names of files
which refer to the same data on LeilFS). Access to the file system can be
limited basing on IP address and/or password.

Distinctive features of LeilFS are:
...
LeilFS installation consists of five types of machines:
 ...
 - metadata backup servers - any number of servers, all of which store metadata
   ...
   leil-master process on such machine when primary master stops working.

 - client computers referring to LeilFS stored files - any number of machines
   with working leil-mount process that communicates with the managing server to
...
leil-chunkserver(8), leil-master(8), leil-metalogger(8), leil-mount(1),
leil-admin(8)
