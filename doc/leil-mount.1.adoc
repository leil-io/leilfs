...
*-o sfswan*::
Apply mount settings tuned for a high-latency link, equivalent to
`sfschunkserverlatencysort=1,sfsacl=0,sfsxattrs=0,sfsdirentrycacheto=60,`
`sfsdirentrycachesize=10000000`. Individual options given after *-o sfswan*
override the preset.

*-o sfsacl=*'0|1'::
Enable or disable ACL handling (default: 1). ACLs are exposed through extended
attributes, so *-o sfsxattrs=0* disables them as well. With ACLs disabled the
client reports no POSIX ACL support to the kernel, hides the ACL extended
attributes from *listxattr*, and answers requests for them with ENOTSUP.

*-o sfsxattrs=*'0|1'::
Enable or disable extended attribute handling (default: 1). With xattrs
disabled *listxattr* reports an empty list and every other extended attribute
request answers ENOTSUP. Useful on high-latency links, where the kernel's
repeated attribute probing dominates the round trip count.
...
*-o sfschunkserverlatencysort=*'0|1'::
When set to 1, prefer chunkservers with a lower observed round trip time when
choosing which replica to read from, including the reads a write performs to
complete a partial stripe (default: 0). Server health still takes precedence,
so a chunkserver recently marked defective is never preferred over a healthy
one no matter how distant the healthy one is.
+
Write targets are not affected: the client writes to the chunkservers the
master supplies, in the order it supplies them.
+
Round trip times are sampled when the client opens a new connection to a
chunkserver, and only when that connection succeeds on its first attempt: one
that succeeded after a retry measures the retry backoff rather than the path.
Connections served from the connection pool are not sampled, and samples do not
decay, so on a steady workload the estimate may rest on comparatively few and
comparatively old observations, and a chunkserver whose connections keep needing
a retry may carry no estimate at all.
